...
class Instance:
    ...
    def check_feasibility(self, solution: Seq[int], obj_value: int) -> FeasibilityCheck:
        ...
        for j, first_item in enumerate(first_items):
            ...
            is_family_split = 1 if len(splits.get(j, ())) > 1 else 0

            if item_count == (family_size := end_item - first_item):
                expected_obj_value += profits[j] - penalties[j] * is_family_split
            elif item_count != 0:
                is_valid = False
                error_messages.append(
        ...
        for j, first_item in enumerate(first_items):
            ...
            for i in range(first_item, end_item):
                ...
                if not isinstance(k, int):
                    ...
                    # if (s := splits.get(j, None)) is None:
                    #    splits[j] = s = set()
                    # s.add(k) TODO forse si può togliere questo pezzo perché non mi interessa quanti split ci sono
                elif k != -1:
                    is_valid = False
                    error_messages.append(
                        f"Item {i} is associated to knapsack k = {k} "
    ...
    def evaluate_solution(self, solution) -> int:
        ...
        for j, first_item in enumerate(first_items):  # Iterate through the families
            ...
            if item_count == (end_item - first_item):  # Check if the family is selected
                is_family_split = 1 if len(splits.get(j, ())) > 1 else 0  # Check if the family is split
                value += profits[j] - penalties[j] * is_family_split  # Increase the value of the solution
        return value
